...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Plot Seigen results')
    p.add_argument('testnames', metavar='testnames', type=str, nargs='+',
                   help='name of results directory')
    args = p.parse_args()
    for i in args.testnames:
        if os.path.isdir(i):
            plot(i)
        else:
            print "Couldn't find results directory %s" % i
